...
export default async function TradeSupportTeamOverviewPage() {
  ...
  return (
    <>
      <PageHeader
        title="Trade Support Team"
        subtitle="Resources and procedures for the trade support team."
      />
      ...
      {categories.length === 0 ? (
        <div className="rounded-3xl border border-gray-200 bg-white p-10 text-center shadow-sm">
          ...
          <p className="mt-2 text-gray-500">
            Add a category with Sidebar Group set to &quot;Trade Support Team&quot; from the admin panel.
          </p>
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2">
          {categories.map((category) => (
            <Link
              key={category.id}
              href={`/TradeSupportTeam/category/${category.slug}`}
