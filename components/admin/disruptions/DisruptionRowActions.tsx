...
import { deleteDisruptionAction } from "@/app/admin/actions/disruption-actions";

interface Props {
  id: number;
}

export default function DisruptionRowActions({ id }: Props) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      try {
        await deleteDisruptionAction(id);
      } catch (error) {
        console.error(error);
