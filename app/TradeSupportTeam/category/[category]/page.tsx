...
export default async function TradeSupportTeamCategoryPage({
  ...
  return (
    <>
      <Breadcrumb
        items={[
          { label: "Trade Support Team", href: "/TradeSupportTeam" },
          { label: categoryRow.name, href: activeFolder ? `/TradeSupportTeam/category/${categoryRow.slug}` : undefined },
      ...
      {folders.length > 0 && (
        <div className="flex flex-wrap gap-3">
          <a
            href={`/TradeSupportTeam/category/${categoryRow.slug}`}
          ...
          {folders.map((f) => (
            <a
              key={f.id}
              href={`/TradeSupportTeam/category/${categoryRow.slug}?folder=${f.id}`}
              className={`rounded-full px-4 py-2 text-sm font-semibold ${
