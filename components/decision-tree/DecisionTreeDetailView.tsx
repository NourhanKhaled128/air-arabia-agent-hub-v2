...
export default async function DecisionTreeDetailView({
  slug,
  basePath = "/decision-trees",
  articleBasePath = "/Knowledge",
}: Props) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      ...
      <div className="rounded-3xl border border-gray-200 dark:border-border-subtle bg-white dark:bg-surface p-8 shadow-sm">
        ...
        {tree.sourceArticle && (
          <p className="mt-4 text-sm text-gray-500 dark:text-slate-400">
            Based on{" "}
            <Link
              href={`${articleBasePath}/${tree.sourceArticle.slug}`}
      ...
      <div>
        <Link href={basePath} className="font-semibold text-red-700 dark:text-brand hover:underline">
