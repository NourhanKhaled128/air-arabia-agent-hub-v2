...
export default function QualityFeedbackManager({ agents, reviews, canManage }: Props) {
  ...
  function handleUpdate(id: number, portalUserId: number, formData: FormData) {
    startTransition(async () => {
      try {
            ...
            {reviews.map((r) =>
              editingId === r.id ? (
                <li key={r.id} className="rounded-xl border border-red-200 p-4">
                  <p className="mb-3 font-semibold text-slate-900">{r.portalUser.name}</p>

                  <form
                    action={(formData) => handleUpdate(r.id, r.portalUser.id, formData)}
                    className="space-y-3"
                  >
