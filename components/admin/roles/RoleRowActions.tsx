...
import { deleteRoleAction } from "@/app/admin/actions/role-actions";

interface Props {
  id: number;
}

export default function RoleRowActions({ id }: Props) {
  const [isPending, startTransition] = useTransition();

  function handleDelete() {
    if (!confirm("Delete this role permanently? Users assigned to it must be reassigned first.")) return;
    ...
    startTransition(async () => {
      try {
        ...
        console.error(error);
        alert("Operation failed. Make sure no users are assigned to this role.");
      }
    });
  }
