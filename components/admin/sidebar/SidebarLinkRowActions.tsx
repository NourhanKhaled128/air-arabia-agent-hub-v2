...
import { deleteSidebarLinkAction } from "@/app/admin/actions/sidebar-actions";

interface Props {
  id: number;
}

export default function SidebarLinkRowActions({ id }: Props) {
  const [isPending, startTransition] = useTransition();

  function handleDelete() {
    if (!confirm("Delete this sidebar link permanently?")) return;

    startTransition(async () => {
      try {
        await deleteSidebarLinkAction(id);
      } catch (error) {
        console.error(error);
